...
- [x] **G13** **♡ page** (added 2026-09-05 at the user's request): a "♡" tab at the far right of the nav opens `/secret`, which asks "What does Haoling like?". Answers are checked client-side against SHA-256 hashes (plain answers are not in the repo). A correct answer sets a session flag and opens `/secret/heart`, a full-screen **Three.js** scene (modeled on the viral 李峋 particle heart): ~11k additive particles sampled from Taubin's heart surface with a lub-dub beat, bloom glow, star field, drag-to-rotate, and a tap that morphs the particles into the name (her name) (text sampled from a canvas); Three.js loads only on this page (~530 KB); direct visits without the flag bounce back to the question. Both pages are `noindex` and excluded from the sitemap. DoD: guard, wrong-answer, and success paths verified with Playwright.
